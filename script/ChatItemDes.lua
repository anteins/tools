...
ChatItemDes = {
	__new_object = function(...)
		return newobject(ChatItemDes, nil, nil, ...);
	end,
	__define_class = function()
		local static = ChatItemDes;
		...
		local instance_methods = {
			JoinSendStr = function(this, obj)
				...
				str = System.String.Format("itemtype=item,itemstaticid={0},itemgid={1}", item.id, serdata.id);
				return str;
			end,
			...
			JoinURLStr = function(this, obj)
				...
				local str; str = System.String.Empty;
				local item; item = LogicStatic.Get__System_Int32(typeof(EightGame.Data.Server.sd_item_item), itemstiticid);
				if (item ~= nil) then
					itemName = item.name;
					itemColor = ChatMsgManager.Instance:GetColorByRank(item.rank);
				end;
		...
		return defineclass(ChatBaseDes, "ChatItemDes", static, static_methods, static_fields_build, static_props, static_events, instance_methods, instance_fields_build, instance_props, instance_events, interfaces, interface_map, false);
	end,
};



ChatItemDes.__define_class();
