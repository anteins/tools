...
ChatFighterDes = {
	__new_object = function(...)
		return newobject(ChatFighterDes, nil, nil, ...);
	end,
	__define_class = function()
		local static = ChatFighterDes;
		...
		local instance_methods = {
			JoinSendStr = function(this, obj)
				...
				str = System.String.Format("itemtype=role,itemstaticid={0},itemgid={1}", role.id, serdata.id);
				return str;
			end,
			...
			JoinURLStr = function(this, obj)
				...
				local str; str = System.String.Empty;
				local role; role = LogicStatic.Get__System_Int32(typeof(EightGame.Data.Server.sd_role_role), itemstiticid);
				if (role ~= nil) then
					itemName = role.rolename;
					itemColor = System.String.Format("[{0}]", ChatMsgManager.Instance:GetColorByRank(role.rank));
				end;
		...
		return defineclass(ChatBaseDes, "ChatFighterDes", static, static_methods, static_fields_build, static_props, static_events, instance_methods, instance_fields_build, instance_props, instance_events, interfaces, interface_map, false);
	end,
};



ChatFighterDes.__define_class();
